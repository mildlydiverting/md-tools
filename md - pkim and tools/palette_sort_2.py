...
class Swatch:
    def __init__(self, hexcode, name=None, group=None):
        self.name = name
        self.group = group
...
def render(sections, path, sw_w=68, sw_h=58, gap=5, pad=26, per_row=16):
    ...
    f_head, f_sub, f_hex, f_nm = font(17, True), font(12, True), font(9), font(8)

    label_h = 24
    width = pad * 2 + per_row * (sw_w + gap) - gap

    def chunk(rows):
        return [rows[i:i + per_row] for i in range(0, len(rows), per_row)] or [[]]
    ...
    for title, blocks in sections:
        ...
        for _, rows in blocks:
            y += 18
            y += len(chunk(rows)) * (sw_h + label_h + gap)
        y += 18
    height = y + pad
    ...
    for title, blocks in sections:
        ...
        for label, rows in blocks:
            d.text((pad, y), f"{label}  ({len(rows)})", font=f_sub, fill="#777777")
            y += 18
            for line in chunk(rows):
                for i, s in enumerate(line):
                    ...
                    d.rectangle([x, y, x + sw_w, y + sw_h], fill=s.hex,
                                outline="#d0d0d0")
                    d.text((x, y + sw_h + 3), s.hex, font=f_hex, fill="#333333")
                    if s.name:
                        d.text((x, y + sw_h + 13),
                               fit(s.name, f_nm, sw_w), font=f_nm, fill="#888888")
                y += sw_h + label_h + gap
        y += 18
